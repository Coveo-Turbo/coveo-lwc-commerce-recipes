 ...
 * @param {string} engineId The ID of the engine.
 */
const initCommerceStore = (engineId) => {
  try {
    ...
    throw new Error(
      'Fatal error: unable to initialize Commerce store: ' + error
    );
  }
};
 ...
 */
function getCommerceStore(engineId) {
...
function registerToStore(engineId, facetType, data) {
  const store = getCommerceStore(engineId);
  try {
...
function getHeadlessBundle(engineId) {
  return window.coveoHeadless[engineId]?.bundle ?? CoveoHeadlessCommerce;
}
