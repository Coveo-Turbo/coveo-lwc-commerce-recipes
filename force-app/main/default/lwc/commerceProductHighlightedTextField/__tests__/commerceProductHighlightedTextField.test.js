...
import CommerceProductHighlightedTextField from '../commerceProductHighlightedTextField';

const componentName = 'c-commerce-product-highlighted-text-field';

const expectedConsoleErrorMessage = `The ${componentName} requires a product and a field to be specified.`;
...
mockHeadlessLoader.initializeWithHeadless = (element, engineId, initialize) => {
  if (element instanceof CommerceProductHighlightedTextField) {
    if (!isInitialized) {
      isInitialized = true;
      initialize();
    }
...
function createTestComponent(options = defaultOptions) {
  const element = createElement(componentName, {
    is: CommerceProductHighlightedTextField,
  });
