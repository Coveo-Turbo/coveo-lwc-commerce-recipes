...
import CommerceSort from 'c/commerceSort';
// @ts-ignore
...
jest.mock('@salesforce/label/c.commerce_SortBy', () => ({default: 'Sort By'}), {
  virtual: true,
});
...
function createTestComponent(options = defaultOptions) {
  ...
  const element = createElement('c-commerce-sort', {
    is: CommerceSort,
  });
...
function mockSuccessfulHeadlessInitialization() {
  ...
  mockHeadlessLoader.initializeWithHeadless = (element, _, initialize) => {
    if (element instanceof CommerceSort && !isInitialized) {
      isInitialized = true;
